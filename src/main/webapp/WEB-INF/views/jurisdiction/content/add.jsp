...
<div id="jurisdictionAddPanel">
	...
	<form class="real-content-form form" name="jurisdiction"
		action="${ctx }/jurisdiction/content/save" method="post">
		<input type="hidden" name="id" value="${jurisdiction.id }">
		<div class="form-group">
			<label class="input-label">名称</label> <input name="name"
				class="input-content" value="${jurisdiction.name }" type="text"
				placeholder="请输入名称">
		</div>
		<div class="form-group">
			<label class="input-label">描述</label>
			<textarea name="description" class="textarea-content"
				placeholder="请输入描述">${jurisdiction.description }</textarea>
		</div>
	</form>
	<div class="bottom-menu-panel">
		<button class="btn btn-bg save-jurisdiction-btn">保存</button>
		<button class="btn btn-bg out-jurisdiction-btn">取消</button>
	</div>
</div>
...
<script>
	$(function() {
		...
		// 保存权限
		$jurisdictionAddPanel.find(".save-jurisdiction-btn").on("click",
				function() {
					$jurisdictionContentForm.trigger("submit");
				});
		//取消保存
		$jurisdictionAddPanel.find(".out-jurisdiction-btn").on("click",
				function() {
					// 刷新页面
					triggerCurrentMenu();
				})
