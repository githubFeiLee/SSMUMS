...
<div id="departmentAddPanel">
	...
	<form class="real-content-form form" name="department" action="${ctx }/department/content/save" method="post">
		...
		<div class="form-group">
			...
			<input name="code" class="input-content" type="text" placeholder="请输入编码" autofocus value="${department.code }">
		</div>
		<div class="form-group">
			<label class="input-label">名称</label> <input name="name" class="input-content"  value="${department.name }" type="text" placeholder="请输入名称">
		</div>
		<div class="form-group">
			<label class="input-label">描述</label>
			<textarea name="description" class="textarea-content" placeholder="请输入描述">${department.description }</textarea>
		</div>
	</form>
	<div class="bottom-menu-panel">
		<button class="btn btn-bg save-department-btn">保存</button>
		<button class="btn btn-bg out-department-btn">取消</button>
	</div>
</div>
...
<script>
	$(function () {
		...
		//取消保存
		$departmentAddPanel.find(".out-department-btn").on("click",function(){
			// 刷新页面
			triggerCurrentMenu();
		})
